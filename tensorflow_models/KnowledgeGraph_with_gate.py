...
class KnowledgeGraph(tf.keras.Model):
    ...
    def call(self, query_vector, global_pointer, gate_signal, kb_len, conv_len, training=True):
        u = [query_vector]  # query_vector: batch_size * embedding_dim.

        # expand gate_signal according kb_len and conv_len.
        # gate_signal: batch_size * 3.
        max_len = global_pointer.shape[1]  # global_pointer: batch_size * max_len.
        gate_signal_list = []
        for bi in range(gate_signal.shape[0]):
            kb_len_t = kb_len[bi] - 1
            conv_len_t = conv_len[bi]
            kb_signal_t = tf.tile(tf.expand_dims(gate_signal[bi, 0], 0), [kb_len_t])
            conv_signal_t = tf.tile(tf.expand_dims(gate_signal[bi, 1], 0), [conv_len_t])
            null_signal_t = tf.expand_dims(gate_signal[bi, 2], 0)
            pad_signal_t = tf.zeros([max_len - kb_len_t - conv_len_t - 1], tf.float32)
            gate_signal_t = tf.concat([kb_signal_t, conv_signal_t, null_signal_t, pad_signal_t], 0)
            gate_signal_list.append(gate_signal_t)
        gate_signal = tf.stack(gate_signal_list, 0)  # gate_signal: batch_size * max_len.

        # how to combine gate_signal with global_pointer?
        global_pointer = global_pointer + gate_signal
