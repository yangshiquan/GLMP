...
class LocalMemoryDecoder(tf.keras.Model):
    ...
    def attend_vocab(self, seq, cond):
        scores_ = tf.matmul(tf.transpose(seq), cond)  # different: no softmax layer, need to check loss function.
        return scores_
    ...
    def call(self, extKnow, story_size, story_lengths, copy_list, encode_hidden,
        ...
        for t in range(max_target_length):
            ...
            query_vector = hidden[0]  # need to check meaning of hidden[0], query_vector: batch_size * embedding_dim.

            p_vocab = self.attend_vocab(self.C.read_value(), tf.squeeze(hidden, 0))  # self.C.read_value: num_vocab * embedding_dim, p_vocab: batch_size * num_vocab.
