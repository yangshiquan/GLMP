...
class CLDataset(data.Dataset):
    ...
    def __init__(self, data_info, src_word2id, trg_word2id):
        ...
        self.num_total_seqs = len(data_info['paired_example'])
        self.src_word2id = src_word2id
        self.trg_word2id = trg_word2id

    def __getitem__(self, index):
        """Returns one data pair (source and target)."""
        paired_example = self.data_info['paired_example'][index]
        paired_example = self.preprocess(paired_example, self.src_word2id, trg=False)
    ...
    def collate_fn(self, data):
        def merge(sequences, story_dim):
            lengths_0 = [len(seq[0]) for seq in sequences]
            lengths_1 = [len(seq[1]) for seq in sequences]
            lengths = []
            for i in range(len(lengths_0)):
                lengths.append(lengths_0[i])
                lengths.append(lengths_1[i])
            max_len = 1 if max(lengths) == 0 else max(lengths)
            if (story_dim):
                padded_seqs = torch.ones(len(sequences), 2, max_len).long()
                for i, seq in enumerate(sequences):
                    # end = lengths[i]
                    # if len(seq) != 0:
                    #     padded_seqs[i, 0, :end] = torch.Tensor(seq[0][:end]).long()
                    #     padded_seqs[i, 1, :(end-1)] = torch.Tensor(seq[1][:(end-1)]).long()
                    padded_seqs[i, 0, :lengths[i*2]] = torch.Tensor(seq[0][:lengths[i*2]]).long()
                    padded_seqs[i, 1, :lengths[i*2+1]] = torch.Tensor(seq[1][:lengths[i*2+1]]).long()
            else:
        ...
        data.sort(key=lambda x: len(x['paired_example'][0]), reverse=True)
        item_info = {}
        for key in data[0].keys():
            item_info[key] = [d[key] for d in data]

        # merge sequences
        paired_example, paired_example_lengths = merge(item_info['paired_example'], True)

        # convert to contiguous and cuda
        paired_example = _cuda(paired_example.view(-1, paired_example.size(-1)).transpose(0,1).contiguous())
        ...
        data_info['paired_example_lengths'] = paired_example_lengths
...
def get_seq_cl(pairs, lang, batch_size, type):
    data_info = {}
    for k in pairs[0].keys():
        data_info[k] = []
    ...
    dataset = CLDataset(data_info, lang.word2index, lang.word2index)
