...
from models.modules_memory_using_kb_arr import *
...
class GLMP(nn.Module):
    ...
    def unk_mask(self, kb_arr, conv_arr):
        kb_size = kb_arr.shape
        rand_mask = np.ones(kb_size, dtype=np.float32)
        bi_mask = np.random.binomial([np.ones((kb_size[0], kb_size[1]), dtype=np.float32)], 1 - self.dropout)[0]
        ...
        conv_rand_mask[:, :, 0] = conv_rand_mask[:, :, 0] * conv_bi_mask
        if USE_CUDA:
    ...
    def encode_and_decode(self, data, max_target_length, use_teacher_forcing, get_decoded_words):
        ...
        if args['unk_mask'] and self.decoder.training:
            story, conv_story = self.unk_mask(data['kb_arr'], data['conv_arr'])
        else:
            story, conv_story = data['kb_arr'], data['conv_arr']
