...
class ContextRNN(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths, hidden=None):
        ...
        mask = self.gen_input_mask(input_seqs.shape[1], input_seqs.shape[0], input_lengths)  # mask: batch_size*max_len
        ...
        outputs = self.bert(input_seqs.transpose(0, 1).type(torch.LongTensor),
                            attention_mask=mask)  # outputs[0]: batch_size*max_len*embedding_dim
        last_hidden_states = outputs[0]  # last_hidden_states: batch_size*max_len*embedding_dim
        hidden = torch.sum(last_hidden_states, dim=1) / max_len  # hidden: batch_size*embedding_dim
